...
module.exports = jtag => {
    ...
    function write(read, rd_len) {
        ...
        if (read) {
            ...
            if (timeout == 0) {
                console.log("Error: timeout " + byte_read + " " + rd_len)
                for (let i = 0; i < byte_read; i++)
        ...
        if (_curr_tms == 0 && nb_byte != 0) {
            ...
            while (nb_byte != 0) {
                ...
                if (_nb_bit + tx_len + 1 > 64) {
                    ...
                    if (writeByte((rx != null)?rx_ptr:null, num_read) < 0)
                        return -1;
                    if (rx != null)
                        rx_ptr += num_read;
                }
    ...
    function writeBit(tdo, nb_bit) {
        let ret = nb_bit; // TODO: write (tdo != null, nb_bit);
        if (tdo != null && ret > 0) {
            for (let i = 0, offset=0; i < nb_bit; i++, offset++) {
    ...
    function writeByte(tdo, nb_byte) {
    ...
    function flush() {
        call_ct = call_ct + 1;
        console.log("---------------------");
        console.log("Flush Call #:" + call_ct);
        console.log("---------------------");
        console.log("END.DR = " + END.DR);
        console.log("END.IR = " + END.IR);
        console.log(`hdr (len, tdo, tdi, mask, smask) = ${hdr.len} ${hdr.tdo} ${hdr.tdi} ${hdr.mask} ${hdr.smask}`);
        console.log(`hir (len, tdo, tdi, mask, smask) = ${hir.len} ${hir.tdo} ${hir.tdi} ${hir.mask} ${hir.smask}`);
        console.log(`sir (len, tdo, tdi, mask, smask) = ${sir.len} ${sir.tdo} ${sir.tdi} ${sir.mask} ${sir.smask}`);
        console.log(`tdr (len, tdo, tdi, mask, smask) = ${tdr.len} ${tdr.tdo} ${tdr.tdi} ${tdr.mask} ${tdr.smask}`);
        console.log(`tir (len, tdo, tdi, mask, smask) = ${tir.len} ${tir.tdo} ${tir.tdi} ${tir.mask} ${tir.smask}`);
        console.log("_tck_pin = " + _tck_pin);
        console.log("_tms_pin = " + _tms_pin);
        console.log("_tdi_pin = " + _tdi_pin);
        console.log("_curr_tms = " + _curr_tms);
        console.log("---------------------");

        //return write(false, 0);
        return 0; // TODO: Remove this and implement write method
    }
