...
module.exports = jtag => {
    ...
    let tir = new svf_XYR();
    let _num_tms = 0;
    let _tms_buffer_size = 128;
    let _tms_buffer = (new Uint8Array(_tms_buffer_size)).fill(0);
    let _run_state = fsm_state.IDLE;
    let _end_state = fsm_state.IDLE;
    let _nb_bit = 0;
    let _buffer_size = 64;
    let _in_buf = (new Uint8Array(_buffer_size)).fill(0);
    let tmpPassRef= new Uint8Array(_tms_buffer_size)
    ...
    function setTMS(tms) {
        ...
        if (tms !== 0)
            _tms_buffer[_num_tms>>3] = _tms_buffer[_num_tms>>3] | (0x1) << (_num_tms & 0x7);
        _num_tms++;
    }
    ...
    function flushTMS(flush_buffer) {
        ...
        if (_num_tms !== 0) {
            ...
            _tms_buffer = _tms_buffer.fill(0);
            _num_tms = 0;
    ...
    function write(read, rd_len) {
        ...
        call_ct += 1;
        //console.log("At write method for iteration : " + call_ct);
        //console.log("Buffer number of bits written = " + _nb_bit);
        //console.log("Input Buffer");
        //console.log("---------------------");
        //for (let i = 0; i < _in_buf.length; i++)
            //console.log(_in_buf[i]);
        //console.log("---------------------");
        ...
        devices.forEach((device) => {
            console.log(`Device: ${device.deviceDescriptor.idVendor.toString(16)}:
        ...
        let ret = 0; // TODO : ll_driver->write(_in_buf, _nb_bit, (read)?_in_buf:NULL, rd_len);
        _nb_bit = 0;
        return ret;
    }
    ...
    function writeTDI (tx, rx, len, last) {
        ...
        if (_curr_tms === 0 && nb_byte !== 0) {
            ...
            while (nb_byte !== 0) {
                ...
                if (_nb_bit + tx_len + 1 > 64) {
                    ...
                    if (writeByte(rx,(rx != null)?rx_ptr:null, num_read) < 0)
                        return -1;
                    if (rx != null)
                        rx_ptr += num_read;
                }
                ...
                if (tx != null) {
                    for (let i = 0; i < tx_len; i++)
                        _in_buf[_nb_bit + i] = tx[tx_ptr + i]
                    // Similar: memcpy(&_in_buf[_nb_bit], tx_ptr, tx_len);
                    tx_ptr += tx_len;
                } else {
                    for (let i = 0; i < tx_len; i++)
                        _in_buf[_nb_bit + i] = 0
                    // Similar: memset(&_in_buf[_nb_bit], 0, tx_len);
                }
                _nb_bit += tx_len;
                nb_byte -= tx_len;
            }
        ...
        if (nb_bit !== 0) {
            ...
            for (let i = 0; i < nb_bit; i++) {
                let val = 0;
                if (tx != null)
                    val = val | ((tx[tx_ptr + (i >> 3)] & (1 << (i & 0x07)))? _tdi_pin : 0);
        ...
        if (last) {
            ...
            tmpPassRef = new Uint8Array(_tms_buffer_size)
            if (writeBit((rx!=null) ? 0:null, 1) < 0)
                return -1;
            if (rx != null)
                rx[rx_ptr] = rx[rx_ptr] | ((tmp & 0x80) >> (7 - nb_bit));
            _in_buf[_nb_bit++] = mask;
    ...
    function writeBit(tdo_ptr, nb_bit) {
        let ret = write (tdo_ptr != null, nb_bit)
        if (tdo_ptr != null && ret > 0) {
            for (let i = 0, offset=tdo_ptr; i < nb_bit; i++, offset++) {
                tmpPassRef[tdo_ptr + ((offset - tdo_ptr) >> 3)] = (((_in_buf[i] & (1<<0)) ? 0x80 : 0x00) |
                    (tmpPassRef[tdo_ptr + ((offset - tdo_ptr) >> 3)] >> 1));
            }
        }
        return ret;
    }

    function writeByte(tdo, tdo_ptr, nb_byte) {
        let ret = write (tdo != null, nb_byte)
        if (tdo !== null && ret > 0)
            for (let i = 0; i < nb_byte; i++) {
                tdo[tdo_ptr + i] = _in_buf[i]
    ...
    function toggleClk1(tms, tdi, clk_len) {
    ...
    function parse_XYR (vector, xyr)   {
        ...
        const new_length = parseInt(vector[1]);
        if (new_length !== xyr.len) {
            clear_svf_XYR(xyr);
        }
        xyr.len = new_length;
        xyr.tdo = '';
        ...
        for (let pos=2; pos < vector.len; pos++) {
            let s = vector[pos];
    ...
    function shiftIR(tdi, tdo, irlen, end_state)  {
        ...
        // C++ implementation covers many devices
        set_state(end_state);
        return 0;
    }
    ...
    function handle_instruction (vector) {
        switch (vector[0]) {
            ...
            case 'TRST':
                ...
                break;
            case 'ENDDR':
                END.DR = fsm_state[vector[1].trim()];
                console.log("enddr value = " + vector[1]);
                state = 'spacelog';
                break;
            case 'ENDIR':
                END.IR = fsm_state[vector[1].trim()];
                console.log("endir value = " + vector[1]);
                state = 'space';
                break;
            case 'STATE':
                newState = fsm_state[vector[1].trim()];
    ...
    function parse_line (line_raw) {
        ...
        if (isComplete) {
            vector.push(...tail.trimStart().split(" "));
